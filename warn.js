...
if (err)  console.log(err);
 ...
 var embed = new discord.MessageEmbed()
   ...
   **Redenen: ** ${reason}`)
   ...
   var channel = message.member.guild.channels.cache.get("857986059031281664");
...
if(warns[warnUser.id].warns == 3){
  ...
  .addField("Bericht", "Je hebt nog een waarschuwing tot je Ban.");

  message.channel.send(embed);

   }else if(warns[warnUser.id].warns == 4){
     message.guild.member(warnUser).ban(reason);
     discord.MessageManager.channel.send(`${warnUser} is verbannen door de bot wegens te veel warns`);
   }
}
